    ...
    <section className='text-gray-600 body-font'>
      <div className='container py-36 mx-auto flex flex-wrap items-center'>
        <div className='lg:w-3/5 md:w-1/2 md:pr-16 lg:pr-0 pr-0'>
          <h1 className='font-medium text-2xl text-center text-white'>Example of custom app develop with DATACAKE graphql api</h1>
        </div>
        <form
          className='lg:w-2/6 md:w-1/2 bg-slate-700 text-white rounded-lg p-8 flex flex-col md:ml-auto w-full mt-10 md:mt-0'
          ...
          <div className='relative mb-4'>
            <label htmlFor={ `${id}-email` } className='leading-7 text-sm>'>
              ...
              <input
                { ...register('email') }
                id={ `${id}-email` }
                className='w-full bg-slate-500 rounded border border-slate-400 text-white focus:border-slate-500 focus:ring-2 focus:ring-slate-700 text-base outline-none py-1 px-3 leading-8 transition-colors duration-200 ease-in-out'
              />
              { errors.email && (
